...
function extractFromZip(buf, filename) {
    ...
    for (let i = 0; i < cdEntries; i++) {
        ...
        if (name.endsWith(filename)) {
            ...
            throw new BinaryNotFoundError(`ZIP entry '${name}' uses compression method ${compMethod} (only stored supported)`);
        }
...
export function findBinary(explicitPath) {
    ...
    if (existsSync(homeBinary))
        return homeBinary;
    throw new BinaryNotFoundError(`${BINARY_NAME} not found in PATH or ~/.trustchain/bin/. ` +
