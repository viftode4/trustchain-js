...
function platformArtifact() {
    ...
    if (!archName)
        throw new BinaryNotFoundError(`Unsupported architecture: ${machine}`);
    if (os === "linux")
        return `trustchain-node-linux-${archName}.tar.gz`;
    if (os === "darwin")
        return `trustchain-node-macos-${archName}.tar.gz`;
    if (os === "win32")
        return `trustchain-node-windows-${archName}.zip`;
 ...
 */
export async function ensureBinary(explicitPath) {
    ...
    if (!asset) {
        ...
        throw new BinaryNotFoundError(`No release artifact '${artifact}' in ${version}. Available: ${available.join(", ")}`);
    }
...
function extractFromZip(buf, filename) {
    ...
    if (eocdOffset < 0)
        throw new BinaryNotFoundError("Invalid ZIP archive");
    ...
    for (let i = 0; i < cdEntries; i++) {
        ...
        if (name.endsWith(filename)) {
            ...
            throw new BinaryNotFoundError(`ZIP entry '${name}' uses unsupported compression method ${compMethod}`);
        }
 ...
 */
export function findBinary(explicitPath) {
    if (explicitPath) {
    ...
    if (existsSync(homeBinary))
        return homeBinary;
    throw new BinaryNotFoundError(`${BINARY_NAME} not found in PATH or ~/.trustchain/bin/. Download from https://github.com/viftode4/trustchain/releases`);
}
 ...
 */
export async function findFreePortBase() {
    const candidates = [];
...
async function checkPortsFree(base, count) {
