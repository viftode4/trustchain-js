 ...
 */
export function defaultEvents(level) {
 ...
 */
export function validateTransaction(schema, transaction) {
    switch (schema) {
        case "base":
 ...
 * Throws if required fields are missing.
 */
export function validateAiActSchema(data) {
    validateBaseSchema(data);
    const missing = [];
    if (data.model_id === undefined)
        missing.push("model_id");
    if (data.input_hash === undefined)
        missing.push("input_hash");
    if (data.output_hash === undefined)
        missing.push("output_hash");
 ...
 * Throws if required fields are missing.
 */
export function validateAiuc1Schema(data) {
    validateBaseSchema(data);
    const missing = [];
